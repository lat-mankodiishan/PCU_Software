...
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *h) {
    if (h == &hcan1) can_mgr_isr_rx_fifo0(CAN_BUS_DRONECAN);
    else if (h == &hcan2) can_mgr_isr_rx_fifo0(CAN_BUS_ENGINE);
}

void HAL_CAN_TxMailbox0CompleteCallback(CAN_HandleTypeDef *h) {
    if (h == &hcan1) can_mgr_isr_tx_complete(CAN_BUS_DRONECAN);
    else if (h == &hcan2) can_mgr_isr_tx_complete(CAN_BUS_ENGINE);
}
