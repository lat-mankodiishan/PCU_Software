...
static const ecu_field_t s_fields[ECU_F_COUNT] = {
    ...
    [ECU_F_SECONDS]      = { "seconds",   0, ECU_FT_U16, 1.0000f,    0.0f }, /* s since boot */
    ...
    [ECU_F_AFR2]         = { "afr2",     30, ECU_FT_S16, 1.0000f,    0.0f }, /* raw already in 0.1 AFR */
...
static void ecu_task(void *arg) {
    ...
    for (;;) {
        ...
        if (ecu_uart_hw_send(&cmd_A, 1, ECU_TX_TIMEOUT_MS) != ECU_UART_OK) {
            ...
            g_pt.ecu_seconds       = (uint16_t)eng[ECU_F_SECONDS];
            g_pt.ecu_coolant_C_x10 = (int16_t) eng[ECU_F_COOLANT];
            g_pt.ecu_tps_pct_x10   = (int16_t) eng[ECU_F_TPS];
            g_pt.ecu_afr1_x10      = (int16_t) eng[ECU_F_AFR1];
            g_pt.ecu_afr2_x10      = (int16_t) eng[ECU_F_AFR2];
            g_pt.ecu_pwmin0        = (uint16_t)eng[ECU_F_PWMIN0];
            g_pt.ecu_pwmin1        = (uint16_t)eng[ECU_F_PWMIN1];
            g_pt.ecu_pwmin2        = (uint16_t)eng[ECU_F_PWMIN2];
            g_pt.ecu_pwmin3        = (uint16_t)eng[ECU_F_PWMIN3];
            g_pt.ecu_rc_throttle   = (uint8_t)(tps_x10 / 10);
