...
uint32_t _uavcan_protocol_dynamic_node_id_server_Discovery_encode(struct uavcan_protocol_dynamic_node_id_server_Discovery* msg, uint8_t* buffer
    ...
    __uavcan_protocol_dynamic_node_id_server_Discovery_encode(buffer, &bit_ofs, msg,
#if CANARD_ENABLE_TAO_OPTION
    tao
#else
    true
...
#endif
    uint32_t bit_ofs = 0;
    if (__uavcan_protocol_dynamic_node_id_server_Discovery_decode(transfer, &bit_ofs, msg,
#if CANARD_ENABLE_TAO_OPTION
    transfer->tao
#else
    true
