...
uint32_t _dsdl_lat_powertrain_ThrottleDemand_encode(struct dsdl_lat_powertrain_ThrottleDemand* msg, uint8_t* buffer
    ...
    uint32_t bit_ofs = 0;
    memset(buffer, 0, DSDL_LAT_POWERTRAIN_THROTTLEDEMAND_MAX_SIZE);
    __dsdl_lat_powertrain_ThrottleDemand_encode(buffer, &bit_ofs, msg,
#if CANARD_ENABLE_TAO_OPTION
    tao
#else
    true
...
#if CANARD_ENABLE_TAO_OPTION
    if (transfer->tao && (transfer->payload_len > DSDL_LAT_POWERTRAIN_THROTTLEDEMAND_MAX_SIZE)) {
        return true; /* invalid payload length */
    }
#endif
    uint32_t bit_ofs = 0;
    if (__dsdl_lat_powertrain_ThrottleDemand_decode(transfer, &bit_ofs, msg,
#if CANARD_ENABLE_TAO_OPTION
    transfer->tao
#else
    true
...
struct dsdl_lat_powertrain_ThrottleDemand sample_dsdl_lat_powertrain_ThrottleDemand_msg(void) {
    ...
    msg.flight_phase = (uint8_t)random_bitlen_unsigned_val(4);
    ...
    msg.throttle_pct = random_float16_val();
